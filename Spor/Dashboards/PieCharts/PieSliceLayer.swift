...
public func ==(slice1: PieSlice, slice2: PieSlice) -> Bool { return slice1.data.id == slice2.data.id }

open class PieSliceLayer: CALayer, CAAnimationDelegate {
    public var color = UIColor.red { didSet { setNeedsDisplay() } }
    
    fileprivate(set) var startAngle = CGFloat(0)
    fileprivate(set) var endAngle = CGFloat(0)
    
    var angles = (CGFloat(0), CGFloat(0)) {
        didSet {
            startAngle = angles.0
            endAngle = angles.1
    ...
    var referenceAngle = CGFloat.pi * 3 / 2 // Top center
    ...
    public var midAngle: CGFloat { return internalMidAngle + referenceAngle }
    
    fileprivate var internalMidAngle: CGFloat { return (startAngle + endAngle) / 2 }
    ...
    public var selected: Bool = false {
        didSet {
            ...
            if let sliceData = sliceData { sliceDelegate?.onSelected(slice: PieSlice(data: sliceData, view: self), selected: selected) }
        }
    }
    ...
    open override func action(forKey event: String) -> CAAction? {
        if disableAnimation { return NSNull() }
        if event == "startAngleManaged" || event == "endAngleManaged" { return makeAnimationForKey(event) }
        return super.action(forKey: event)
    }
    
    open override class func needsDisplay(forKey key: String) -> Bool {
        return (key == "startAngleManaged" || key == "endAngleManaged") ? true : super.needsDisplay(forKey: key)
    }
...
public struct PieSlice: Hashable, CustomDebugStringConvertible {
    ...
    public var hashValue: Int { return data.id }
    public var debugDescription: String { return data.debugDescription  }
}
...
public class PieSliceModel: CustomDebugStringConvertible {
    ...
    public init(value: Double, description: String, color: UIColor, obj: Any? = nil) {
        self.value = value
        self.color = color
